...
def gamma(gas: 'gas.Gas', altitude: float):
    ...
    return t_frac * (gas.gamma_air *
                     (pressure - p_self) + gas.gamma_self * p_self)


def lorentzian_fit(gas: 'gas.Gas',
                   altitude: float,
    ...
    '''
    peaks = main_peaks(gas, start=start, stop=stop,
                       quantile=threshold)  #find peaks for gas
