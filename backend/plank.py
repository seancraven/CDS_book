...
def plank(x, temperature: int, flux: bool=False, units='cm', is_wavelength=True):
...
def plank_lambda(wavelength, temperature: int, flux=False, units='cm'):
    ...
    return pifac * c_1 / wavelength**5 * (1/(np.exp(c_2 / (wavelength*temperature)) - 1))

def plank_nu(nu_, temperature: int, flux=False, units='cm'):
    ''' Plank Function as a function of frequency
        Args:
            nu_: an array or list, in wavlength or frequency(nu)
            temperature: temperature in K
            Flux: Boolean Value for False returns radiance
    ...
    if flux: ### Returns Flux in w/(m^2 cm^-1)
        ...
        pifac = 1
    return pifac * c_1 *nu_**3 / (np.exp(c_2*nu_/temperature) - 1) * 10**6
