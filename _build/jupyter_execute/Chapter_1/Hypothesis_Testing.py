...
# The $P(\chi^2 \geq \chi^2_{min};\nu) = 0.0$, values are infact expected. On inspection, the error on each measurement is of the order $0.1$ ppm. However, the magnitude of the residuals regularly is of the order $1$ ppm. As such, the fit to the Mauna Loa Data should be poor. The third order polynomial across both global and Mauna Loa data-sets provides much better agreement. This will be the model taken forward. 
# 
